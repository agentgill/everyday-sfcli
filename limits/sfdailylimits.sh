...
#capture stdout
dxoutput=$(date ; sfdx force:limits:api:display $adminuser | grep -v 'Concurrent' | grep -v 'Hourly' | grep -v 'SingleEmail'  | grep -v 'MassEmail' | grep -v 'Storage' | grep -v 'PermissionSets')
